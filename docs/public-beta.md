...
isolated environment, install the package, and run `svgai-marketing doctor`. Add `[browser]` only for explicit
...
When reporting a problem, include `svgai-marketing doctor --json`, the command used, the output status/reason,
